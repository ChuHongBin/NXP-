...
namespace Camerasimulation
{
    public partial class FormMain : Form
    {
        ...
        {
            //允许图片控件拖放
            labal_pictureBox_Preview.AllowDrop = true;
            ...
            ShowImageColor.LeftLine = Color.Red;/// Color.Yellow;
            ShowImageColor.RightLine = Color.Red;//// Color.Yellow;
            ShowImageColor.MiddleLine = Color.Red;
            ShowImageColor.NormalLine = Color.Green;
            ShowImageColor.normline = Color.Yellow;
        }
        ...
        private void ToolStripMenu_Clear_Click(object sender, EventArgs e)
        {
            ...
            //显示空图片
            labal_pictureBox_Preview.Image = null;
        }
        ...
        {
            //鼠标在PictureBox中的坐标
            Point CursorPos = labal_pictureBox_Preview.PointToClient(MousePosition);

            //转换为图片分辨率下的坐标
            CursorPos.X = (int)(CursorPos.X * RealSize.Width / labal_pictureBox_Preview.Width);
            CursorPos.Y =RealSize.Height - 1 - (int)(CursorPos.Y * RealSize.Height / labal_pictureBox_Preview.Height);
        ...
        private void pictureBox_Preview_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (checkedListBox_ShownOptions.GetItemChecked(2) == true)
            {
                ...
                for (int x = 1; x < RealSize.Width; x++)
                {
                    g.DrawLine(
                        ...
                        new Point(x * labal_pictureBox_Preview.Width / RealSize.Width, 0),
                        new Point(x * labal_pictureBox_Preview.Width / RealSize.Width, labal_pictureBox_Preview.Height));
                }
                for (int y = 1; y < RealSize.Width; y++)
                {
                    g.DrawLine(
                        new Pen(Brushes.Black),
                        new Point(0, y * labal_pictureBox_Preview.Height / RealSize.Height),
                        new Point(labal_pictureBox_Preview.Width, y * labal_pictureBox_Preview.Height / RealSize.Height));
                }
            }
            //显示法线
            if (checkedListBox_ShownOptions.GetItemChecked(3) == true)
            {
                ...
                g.DrawLine(
                    ...
                    new Point(labal_pictureBox_Preview.Width / 2, 0),
                    new Point(labal_pictureBox_Preview.Width / 2, labal_pictureBox_Preview.Height));
                if (labal_pictureBox_Preview.Width % 2 == 0)
                {
                    g.DrawLine(
                        new Pen(ShowImageColor.NormalLine),
                        new Point(labal_pictureBox_Preview.Width / 2 - 1, 0),
                        new Point(labal_pictureBox_Preview.Width / 2 - 1, labal_pictureBox_Preview.Height));
                }
            }
